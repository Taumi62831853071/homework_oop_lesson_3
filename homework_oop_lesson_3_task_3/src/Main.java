...
public abstract class Main {
    public static void main(String[] args) {
        ...
        Mammal.Herbivore gazelle = new Mammal.Herbivore(
                "Газель",
                1,
                "Полупустыни и пустыни",
                50,
                "Трава, листья и молодые ветки кустарников"
        );
        Mammal.Herbivore giraffe = new Mammal.Herbivore(
                "Жираф",
                2,
                "Тропические степи с редкорастущими деревьями",
                55,
                "Листья, цветы, семена, фрукты"
        );
        Mammal.Herbivore horse = new Mammal.Herbivore(
                "Лошадь",
                3,
                "",
                25,
                "Трава, фрукты, овощи, листья, корни, злаки"
        );
        ...
        Mammal.Predator hyena = new Mammal.Predator(
                "Гиена",
                4,
                "Африка",
                60,
        ...
        Mammal.Predator tiger = new Mammal.Predator(
                "Тигр",
                5,
                "Влажные тропические леса, мангровые болота, бамбуковые чащи в тропиках, сухие саванны, " +
                        "полупустыни, голые каменистые сопки и тайга на севере",
                60,
                "Копытные"
        );
        Mammal.Predator bear = new Mammal.Predator(
                "Медведь",
                6,
        ...
        Amphibian frog = new Amphibian(
                "Лягушка",
                0,
                "Пресноводные водоёмы, кроны деревьев, подземные норы"
        );
        Amphibian alreadyFreshwater = new Amphibian(
                "Уж пресноводный",
                0,
                "От Средней и Южной Европы до Западного Китая и Северо-Западной Индии"
        );
        ...
        Bird.Flightless peacock = new Bird.Flightless(
                "Павлин",
                2,
                "Леса и саванны, на полуострове Индостан и на острове Цейлон","Ходьба"
        );
        Bird.Flightless penguin = new Bird.Flightless(
                "Пингвин",
                1,
                "Открытое море Южного полушария: в прибрежных водах Антарктики, Новой Зеландии," +
                        " южной части Австралии, Южной Африке, по всему западному побережью Южной Америки",
                "Ходьба, плавание"
        );
        Bird.Flightless dodo = new Bird.Flightless(
                "Птица Додо",
                0,
                "",
                "Ходьба"
        ...
        Bird.Flying seagull = new Bird.Flying(
                "Чайка",
                1,
                "Везде, где есть открытый выход к морю или океану",
                "Полёт"
        );
        Bird.Flying albatross = new Bird.Flying(
                "Альбатрос",
                2,
                "Ледяные воды Антарктиды и и вообще всё Южное полушарие",
                "Полёт"
        );
        Bird.Flying falcon = new Bird.Flying(
                "Сокол",
                3,
                "пустыни, тундры, тайга, луга, саванны, кустарниковые леса, горах, прибрежных районы, " +
                        "водно-болотные угодья, устья рек, берега озер, сельскохозяйственные районы, пригороды и города",
                "Полёт"
        );
    ...
    private static void printInfoHerbivore(Mammal.Herbivore mammal) {
        System.out.println(mammal.name + ", колличество лет: " + mammal.age + ", среда обитания: " + mammal.habitat +
                ", скорость: " + mammal.speed + " км/ч, тип пищи: " + mammal.foodType);
    }
    ...
    private static void printInfoAmphibian(Amphibian amphibian) {
        System.out.println(amphibian.name + ", колличество лет: " + amphibian.age + ", среда обитания: " +
                amphibian.habitat);
    }

    private static void printInfoBird(Bird.Flightless bird) {
        System.out.println(bird.name + ", колличество лет: " + bird.age + ", среда обитания: " + bird.habitat +
                " , тип передвижения: " + bird.movementType);
    }
